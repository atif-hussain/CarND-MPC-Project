    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double px = j[1]["x"];
          double py = j[1]["y"];
          double psi = j[1]["psi"]; // psi/= 25.*M_PI/180;
          double v = j[1]["speed"];
          cout << "Current state = (x=" << px << ",y=" << py << ",psi=" << psi << ",v=" << v << ")" << endl; 

          /*
          * TODO: Calculate steering angle and throttle using MPC.
          *
          * Both are in between [-1, 1].
          * Now converted to SI units.
          */
          ...
          /* Convert units */
          std::cout << "current cte: " << cte << "m?"  << "velocity: " << v << "mph" << endl;
          // here if converting from mph to m/s
          // v = v * 1609 / 3600;
          ...
          double delta = j[1]["steering_angle"]; // mpc.prev_delta; // 
          ...
          cout << "predicted state = (x=" << predicted_x << ",y=" << predicted_y << ",psi=" << predicted_psi << ",v=" << predicted_v << ",cte=" << predicted_cte << ",epse=" << predicted_epsi << ")" << endl; 
          
          // Solve using MPC
          // coeffs to predict future cte and epsi
          auto result = mpc.Solve(state, coeffs);
          
          double steer_value = - result[0] ; //   1/(deg2rad(25)*Lf);
          std::cout << "steer_value: " << steer_value << endl;
          double throttle_value = result[1];
          
          // Store previous actuator values, used to predict future state
